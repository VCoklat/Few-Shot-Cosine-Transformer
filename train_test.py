...
if __name__=="__main__":
    ...
    base_ldr = base_mgr.get_data_loader(base_json, aug=p.train_aug)
    val_ldr  = val_mgr .get_data_loader(val_json , aug=False)

    # --------------------- Model ----------------------------------- #
    if "FSCT" in p.method:
        var = "cosine" if "cosine" in p.method else "softmax"
        feat = lambda: build_feature(p.backbone, p, flatten=True)
        model = FewShotTransformer(feat, variant=var, **params)
    elif "CTX" in p.method:
        var = "cosine" if "cosine" in p.method else "softmax"
        feat = lambda: build_feature(p.backbone, p, flatten=False)
        model = CTX(feat, variant=var,
