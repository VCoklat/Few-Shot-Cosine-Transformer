...
def get_model_for_dataset(dataset, model_func, n_way, k_shot, n_query,
                          feature_dim=64, n_heads=4, dropout=0.1):
    """
    Factory function to get optimally configured model for each dataset.
    
    Args:
        dataset: Dataset name ('omniglot', 'miniimagenet', 'ham10000', 'cub')
        model_func: Function that returns backbone network
    ...
    Returns:
        EnhancedOptimalFewShot model with dataset-specific configuration
    """
    ...
    config = configs.get(dataset_lower, configs['miniimagenet'])
    
    # Override dropout if provided
    if dropout != 0.1:
